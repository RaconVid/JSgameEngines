(function(){
	...
	function Collider(SpaceLayer=this.SpaceLayer){//spriteRef=Collider.call(sprite).addPhysics(layer...);
		...
		this.coords=[0,0];
		this.velocity=[0,0];
		this.matrixPos=[[1,0,0],[0,1,0],[0,0,1]];//[rotation,coords]
	...
	mainGame.camera={matrixPos:[],coords:[0,0],matrixPosI:[[1,0,0],[0,1,0],[0,0,1]]};
	mainGame.camera.matrixPos=Clone(mainGame.camera.matrixPosI);
	function Camera(SpaceLayer=this.SpaceLayer){//addCamera
		...
		this.drawScript=new mainGame.UpdateScript(this,layers.draw[4],undefined,function(){
			...
			mainGame.camera.coords[2]=Math.addVec(
				[-this.sprite.coords[0],-this.sprite.coords[1],0],
				mainGame.camera.coords
			);
			Draw.undoTranslate(this.sprite.matrixPos);
			mainGame.camera.matrixPos=Math.timesMatrix(
				mainGame.camera.matrixPos,
				this.sprite.matrixPos
			);
			
			this.sprite.drawLayer.onUpdate();
			
			mainGame.camera.matrixPos=Math.timesMatrix(
				mainGame.camera.matrixPos,
				Math.inverseMatrix(this.sprite.matrixPos),
			);
			Draw.translate(this.sprite.matrixPos);
			mainGame.camera.coords[2]=Math.minusVec(
				[this.sprite.coords[0],this.sprite.coords[1],0],
				mainGame.camera.coords
			);
			ctx.translate(this.sprite.coords[0],this.sprite.coords[1]);
