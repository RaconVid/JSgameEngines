...
if(1){
	let makeEvent=(length,coords,rotation)=>new Space.Sprite({
		OnStart(){
			this.attach();
		},
		...{
		...
		scripts:{
			...
			mainDraw:{attach:true,layer:l=>l.draw[8],script(layer,script){
					ctx.save();{
						...
						ctx.fillStyle="blue";
						let r=20,r1=r*1.2;
						let len=[this.length*r1,r1];
						ctx.fillRect(-len[0]/2,-len[1]/2,...len);
						ctx.save();
						ctx.translate(r1*(this.length/2-0.5),0);
						for(let i=0;i<this.length;i++){
							Draw.circle(0,0,r/2,"#37FF37");
							ctx.translate(-r1,0);
						}
						ctx.restore();
					}ctx.restore();
